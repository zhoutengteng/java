 ...
 */
public class Jigsaw {
	static public int staticCount = 0;
	...
	public boolean BFSearch() throws IOException {
        ...
        while (openList != null &&!openList.isEmpty()) {
        	currentJNode = openList.firstElement();
        	if (Arrays.equals(currentJNode.getNodesState(), endJNode.getNodesState())) {
        	    isCompleted = true;
				//this.calSolutionPath();
        		JigsawNode currentJNode1 = this.currentJNode;
        	    solutionPath = new Vector<JigsawNode>();
        	    while (currentJNode1 != null) {
        	    	solutionPath.add(currentJNode1);
        	    	currentJNode1 = currentJNode1.getParent();
        	    }
        	    break;
        	...
        	Vector<JigsawNode> four = findFollowJNodes(currentJNode);
            openList.remove(0);
			// 记录并显示搜索过程
            ...
            if (four != null) openList.addAll(four);
        	if (currentJNode != null) closeList.add(currentJNode);
            searchedNodesNum++;
        }
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for (int index = 1; index <= dimension * dimension; index++) {
			if (jNode.getNodesState()[index] != 0) {
				int jNodeRow = (jNode.getNodesState()[index] - 1)  / dimension;
			    int jNodeCol = (jNode.getNodesState()[index] - 1)  % dimension;
			    int indexRow = (index - 1) / dimension;
			    int indexCol = (index - 1) % dimension;
			    instance += Math.abs(jNodeRow - indexRow) + Math.abs(jNodeCol - indexCol);
			    instance2 += Math.sqrt((jNodeRow - indexRow) * (jNodeRow - indexRow) + (jNodeCol - indexCol) * (jNodeCol - indexCol));
			}
		}
        int count = 0;
        for (int index = 1; index < dimension * dimension; index++) {
		...
		jNode.setEstimatedValue(count + s * 3 + instance * 2 + instance2 * 1);
	}
